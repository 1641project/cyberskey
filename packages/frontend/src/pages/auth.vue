...
function accepted() {
	state = 'accepted';
	const getUrlParams = () => window.location.search.substring(1).split('&').reduce((result: any, query) => { const [k, v] = query.split('='); result[k] = decodeURI(v); return result; }, {});
	...
	if (session && session.app.callbackUrl && isMastodon) {
		...
		const tokenRaw = session.token
		const token = tokenRaw.replaceAll('-', '')
		location.href = `${session.app.callbackUrl}?code=${token}${stateParam}`;
